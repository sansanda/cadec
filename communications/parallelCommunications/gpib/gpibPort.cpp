...
{
    if (addr<=0 || addr>31) setAddress(1);
    setAddress(addr);
}
...
int GPIBPort::setTimeout (int timo) {return ibtmo(BOARD_INDEX,timo);}
...
int GPIBPort::openConnection(int eos){
    ...
    qDebug()<<"K24XX: Opening the instrument";
    ...
    if( isNoError() ){
        ...
        device = ibdev(BOARD_INDEX, address, SAD, TIMEOUT_3s, EOT, eos);
        //std::cout<<"****************************************************************************"<<device<<std::endl;
        status = errors();
    }
    if( isNoError() ){
        ibln( 0, address, SAD, &listen);
        //std::cout<<"****************************************************************************"<<a<<std::endl;
        status = errors();
    }
#endif
